...
<?php
    ...
    $sql = "SELECT b.boardTitle, m.youName, b.regTime, b.boardView, b.boardContents FROM Community b JOIN blog_myMembers m ON(b.memberId = m.memberId) WHERE b.boardId = {$boardId}";
    $result = $connect -> query($sql);

    if ($result) {
        $info = $result->fetch_array(MYSQLI_ASSOC);
        
        echo '<div class="info-item">'; // 게시글 제목
        echo '<span class="title">' . $info['boardTitle'] . '</span>';
        echo '</div>';
    
        echo '<div class="info-item">'; // 작성자
        echo '<span class="name">' . $info['youName'] . '</span>';
        echo '</div>';
    
        echo '<div class="info-item">'; // icon1 = 작성날짜, icon2 = 조회수 , icon3 = 댓글
        echo '<img class="icon1" src="../assets/img/clock.svg" alt="Clock Icon"><span class="small-text">' . date('Y-m-d', $info['regTime']) . '</span>';
        echo '<img class="icon2" src="../assets/img/read1.svg" alt="Read Icon"><span class="small-text">' . $info['boardView'] . '</span>';
        echo '<img class="icon3" src="../assets/img/chat.svg" alt="Read Icon"><span class="small-text">' . $info['comment'] . '</span>';
        echo '</div>';
    }
?>
            </div>
